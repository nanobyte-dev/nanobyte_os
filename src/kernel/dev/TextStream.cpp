...
TextStream::TextStream(CharacterDevice* dev)
    : m_dev(dev)
{
}

bool TextStream::Write(char c)
{
    return m_dev->Write(reinterpret_cast<const uint8_t*>(&c), sizeof(c)) == sizeof(c);
}

bool TextStream::Write(const char* str)
{
    bool ok = true;
...
bool TextStream::VFormat(const char* fmt, va_list args)
{
...
bool TextStream::Format(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
bool TextStream::FormatBuffer(const char* msg, const void* buffer, size_t count)
{
